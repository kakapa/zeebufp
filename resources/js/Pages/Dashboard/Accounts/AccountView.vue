  ...
  <div class="p-2 space-y-4">
    <div class="flex flex-col gap-4">
      ...
      <div class="pt-2">
        ...
        <div class="space-y-2">
          <div
            v-for="(pkg, index) in account.additionalPackages"
            :key="pkg.id"
            class="border rounded-md"
          >
            <button
              @click="toggleAccordion(pkg.id)"
              class="flex items-center justify-between w-full p-3 text-left hover:bg-gray-50"
            >
              <div class="flex items-center space-x-3">
                <span class="font-medium">{{ pkg.name }}</span>
                <span class="text-sm text-gray-500"
                  >{{ pkg.monthlyContributionString }}pm</span
                >
              </div>
              <svg
                class="w-5 h-5 text-gray-500 transition-transform duration-200"
                :class="{ 'rotate-180': openAccordion === pkg.id }"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div
              v-show="openAccordion === pkg.id"
              class="p-3 pt-0 mb-3 border-t"
            >
              <div class="grid grid-cols-1 md:grid-cols-1 gap-3 text-sm">
                <div>
                ...
                <div>
                  ...
                  <p>{{ pkg.features || "N/A" }}</p>
                </div>
                <div>
                  <p class="text-gray-500">Description</p>
                  <p>{{ pkg.description || "N/A" }}</p>
                </div>
                <div>
          ...
          <p
            v-if="account.additionalPackages.length === 0"
            class="text-sm text-gray-500"
          >
            No addtional packages listed
          </p>
        </div>
      </div>
...
const deletePackage = (packageId) => {
  ...
  router.delete(
    route("accounts.packages.detach", {
      account: props.account.id, // The account ID
      package_id: packageId, // The package ID to detach
    }),
    {
      preserveScroll: true,
      onSuccess: () => {
        ...
        // Remove the package from the account's additionalPackages list
        const index = props.account.additionalPackages.findIndex(
          (p) => p.id === packageId
        );
        if (index !== -1) {
          props.account.additionalPackages.splice(index, 1);
        }
      },
      onError: (error) => {
        // Optional: Show error message
